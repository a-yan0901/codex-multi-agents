...
把所有任务都丢在同一个 Codex 会话里跑，意味着每次都要带上完整的系统提示词、统一的工具列表和通用指令，对绝大多数真实工作来说都是浪费。

本包把工作拆给六个专精化的子 Agent —— explorer、architect、worker、tester、debugger、reviewer —— 每个都有简短、聚焦于角色的系统提示词，以及为该任务调优的模型和推理力度。主 Codex 会话读懂请求后，先判断它属于哪类工作（快速搜索、方案设计、代码改动、失败的测试、复杂故障、风险较高的改动），再委派给对应的子 Agent。

这样做有两个直接收益：

- **降低 token 消耗。** 子 Agent 的提示词只提到自己需要的工具和策略，orchestrator 不必在每轮都付出完整通用提示词的开销。
- **职责分离更清晰。** 探索、设计、实现、验证、review 各自跑在独立的上下文里，调研结果不会污染实现，反之亦然。
